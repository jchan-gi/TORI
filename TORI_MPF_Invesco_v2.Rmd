...
2020-03-27 v2.1.1a     
1. Fix return matrix column misalignment in Tori @ Invesco.    
...
MPF.Invesco <-
  as.xts(
    read.zoo(
      paste(Sys.getenv("onedrive"),"/MPF/Invesco/Invesco.csv",sep=""),
      format = "%d/%m/%Y",
      read = read.csv,
      na.strings = "0"
    )
...
```{r warning=FALSE}
MPF.Invesco.SMA.hy <-
  na.fill(apply(MPF.Invesco, 2, function(x)
...
MPF.Invesco.HKCE <-
  monthlyReturn(as.xts(MPF.Invesco$HKCE), type = "log")
MPF.Invesco.HSI <-
  monthlyReturn(as.xts(MPF.Invesco$HSI), type = "log")
MPF.Invesco.MPFC <-
  monthlyReturn(as.xts(MPF.Invesco$MPFC), type = "log")
MPF.Invesco.RMBB <-
  monthlyReturn(as.xts(MPF.Invesco$RMBB), type = "log")
MPF.Invesco.S <- monthlyReturn(as.xts(MPF.Invesco$S), type = "log")
MPF.Invesco.SFP <-
  monthlyReturn(as.xts(MPF.Invesco$SFP), type = "log")
...
MPF.Invesco.AE <-
  monthlyReturn(as.xts(MPF.Invesco$AE), type = "arithmetic")
MPF.Invesco.B <-
  monthlyReturn(as.xts(MPF.Invesco$B), type = "arithmetic")
MPF.Invesco.CA <-
  monthlyReturn(as.xts(MPF.Invesco$CA), type = "arithmetic")
MPF.Invesco.G <-
  monthlyReturn(as.xts(MPF.Invesco$G), type = "arithmetic")
MPF.Invesco.GB <-
  monthlyReturn(as.xts(MPF.Invesco$GB), type = "arithmetic")
MPF.Invesco.HKCE <-
  monthlyReturn(as.xts(MPF.Invesco$HKCE), type = "arithmetic")
MPF.Invesco.HSI <-
  monthlyReturn(as.xts(MPF.Invesco$HSI), type = "arithmetic")
MPF.Invesco.MPFC <-
  monthlyReturn(as.xts(MPF.Invesco$MPFC), type = "arithmetic")
MPF.Invesco.RMBB <-
  monthlyReturn(as.xts(MPF.Invesco$RMBB), type = "arithmetic")
MPF.Invesco.S <-
  monthlyReturn(as.xts(MPF.Invesco$S), type = "arithmetic")
MPF.Invesco.SFP <-
  monthlyReturn(as.xts(MPF.Invesco$SFP), type = "arithmetic")
...
colnames(MPF.Invesco.simple.returns) <-
  c(
    "Asia.Equity",
    'Balance',
    'Core.accumulation',
...
```{r warning=FALSE}
MPF.Invesco.RSI.month <-
  as.xts(do.call(rbind, lapply(split(as.xts(MPF.Invesco.RSI), "months"), function(x)
...
for (col in 1:length(MPF.Invesco.RSI.month[1, ])) {
  if (col != 8) {
    for (row in 1:length(MPF.Invesco.RSI.month[, col])) {
      ...
      if (percentile(MPF.Invesco.RSI.month[row, col]) >= (RSI_Overbuy - 1 /
                                                          length(1:row) ^ (1 / 2))) {
        MPF.Invesco.RSI.p[row, col] <- 0.2
      } else {
        MPF.Invesco.RSI.p[row, col] <-
          1.2 - (percentile(MPF.Invesco.RSI.month[row, col]) ^ (3))
      }
    }
  } else {
...
MPF.Invesco.RSI.sum <-
  as.xts(rowSums(MPF.Invesco.RSI.p), order.by = monthly)

for (row in 1:length(MPF.Invesco.RSI.p[, col])) {
  MPF.Invesco.RSI.p[row, ] <-
    apply(MPF.Invesco.RSI.p[row, ], 2, function(x)
      (x / MPF.Invesco.RSI.sum[row, 1]) ^ (1 / 4))
}
MPF.Invesco.RSI.sum <-
  as.xts(rowSums(MPF.Invesco.RSI.p), order.by = monthly) 
...
```{r warning=FALSE}
rm(
  MPF.Invesco.BIAS.diff,
  MPF.Invesco.BIAS.diff.qu,
  MPF.Invesco.BIAS.month,
  MPF.Invesco.BIAS.p
)
MPF.Invesco.BIAS.diff <-
  (MPF.Invesco.SMA.mt - MPF.Invesco.SMA.hy) / MPF.Invesco.SMA.hy
MPF.Invesco.BIAS.diff.qu <-
  MPF.Invesco.BIAS.diff - stats::lag(MPF.Invesco.BIAS.diff, k = 21 * nm)
MPF.Invesco.BIAS.diff.qu <- na.fill(MPF.Invesco.BIAS.diff.qu, 0)


MPF.Invesco.BIAS.month <-
  do.call(rbind, lapply(split(
    as.xts(MPF.Invesco.BIAS.diff.qu), "months"
  ), function(x)
    colAvgs(x)))
...
MPF.Invesco.stock.return <-
  as.xts(rowSums(MPF.Invesco.BIAS.month), order.by = monthly)
...
for (row in 1:length(MPF.Invesco.BIAS.month[, 1])) {
  MPF.Invesco.BIAS.month[row, ] <-
    (MPF.Invesco.BIAS.month[row, ]) * MPF.Invesco.RSI.p[row, ]
  ...
  for (col in 1:length(MPF.Invesco.BIAS.month[1, ])) {
    if (col != 2 &&
        col != 3 &&
        col != 5 && col != 8 && col != 9 && col != 10 && col != 11) {
      if (!is.na(MPF.Invesco.returns.mat[row, col]) &&
          MPF.Invesco.returns.mat[row, col] != 0) {
        MPF.Invesco.stock.mean <-
          MPF.Invesco.stock.mean + MPF.Invesco.returns.mat[row, col]
        i <- i + 1
      }
  ...
  if (row <= 12 ||
      MPF.Invesco.sum == MPF.Invesco.BIAS.month[row, 8] ||
      MPF.Invesco.sum < 1e-6 || hedge == TRUE) {
    if (row >= 198) {
      MPF.Invesco.BIAS.p[11] <- 0.4
      MPF.Invesco.BIAS.p[8] <- 0.3
      MPF.Invesco.BIAS.p[5] <- 0.3
    } else if (row >= 29) {
      MPF.Invesco.BIAS.p[8] <- 0.5
      MPF.Invesco.BIAS.p[5] <- 0.5
    } else {
    ...
    if (row >= 198) {
      MPF.Invesco.BIAS.p[] <-
        MPF.Invesco.BIAS.month[row,] / MPF.Invesco.sum / 10 * 7
      MPF.Invesco.BIAS.p[5] <- MPF.Invesco.BIAS.p[5] + 0.1
      MPF.Invesco.BIAS.p[8] <- MPF.Invesco.BIAS.p[8] + 0.08
      MPF.Invesco.BIAS.p[11] <- MPF.Invesco.BIAS.p[11] + 0.12
    } else if (row >= 29) {
      MPF.Invesco.BIAS.p[] <-
        MPF.Invesco.BIAS.month[row,] / MPF.Invesco.sum / 10 * 7
      MPF.Invesco.BIAS.p[5] <- MPF.Invesco.BIAS.p[5] + 0.18
      MPF.Invesco.BIAS.p[8] <- MPF.Invesco.BIAS.p[8] + 0.12
    } else {
      MPF.Invesco.BIAS.p[] <-
        MPF.Invesco.BIAS.month[row,] / MPF.Invesco.sum / 10 * 7
    ...
    MPF.Invesco.BIAS.p[] <-
      MPF.Invesco.BIAS.month[row,] / MPF.Invesco.sum
  }
  ...
  if (row == 1) {
    portf.rebal.fm <- na.fill(MPF.Invesco.simple.returns[row, ], 0) *
      MPF.portf.weight[row, ]
  } else {
    portf.rebal.fm <-
      Return.portfolio(
        na.fill(MPF.Invesco.simple.returns[1:row, ], 0),
...
portf.rebal.fm.dollar.gb <- Return.portfolio(
  tail(MPF.Invesco.simple.returns,-46),
...
ggplot() +
  scale_color_manual(
    labels = c("SPY", "QQQ", "HSI", "TORI @ Invesco"),
  ...
  geom_line(aes(x = date, y = Ra, color = "pink3"), portf.dollar.monthly, 
            size =1.8) +
  labs(
    title = "TORI @ Invesco versus SPY, QQQ, and Hang Seng Index",
    subtitle = "How TORI @ Invesco outperform common index fund",
    x = "",
    y = "Portfolio Value",
...
ggplot() +
  scale_color_manual(labels = c("Golden Butterfly", "TORI @ Invesco"),
                     values = c(4, "pink3")) +
  ...
  geom_line(aes(x = date, y = Ra, color = "pink3"),
            ...
            size = 1.8) +
  labs(
    title = "TORI @ Invesco versus Golden Butterfly Portfolio",
    subtitle = "How TORI @ Invesco outperform golden butterfly portfolio",
    x = "",
    y = "Portfolio Value",
    color = "Portfolio"
  ) +
...
kable(df, "pandoc",
      col.names = c("指標 Indicator", "Tori @ Invesco", "SPY", "HSI", "Golden Butterfly"))
```
