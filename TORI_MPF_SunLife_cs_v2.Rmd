...
年率化回報 Annualized Return: ~11.11%     
算術年均回報 Mean Annual Return: ~10.61%       
累積回報 Cumulative Return: ~583.72%    
年率化標準誤差 Annualized Standard Deviation: ~8.91% (StdDev(monthly return) * sqrt(12))    
夏普比率 Sharpe Ratio: 1.2305          
索丁諾比率 Sortino Ratio: 3.2452 (MAR = 0%)      
預期損失 Expected Shortfall: 3.93% loss (0% Risk-free rate, 95% C.I.)       

月供投資 Monthly MPF installment (或有誤差 maybe slightly differs):    
每月供款 Monthly installment amount: 2301    
總供款 Total contribution: 506220    
最新結餘 Latest asset value: 1387602        
算術年均回報 Mean annual return: 10.10%      
內部回報率 Internal Rate of Return (IRR): 9.58%    
年率化標準誤差 Annualized Standard Deviation: 8.90%    
夏普比率 Sharpe Ratio: 1.1356          
索丁諾比率 Sortino Ratio: 2.9428 (MAR = 0%)    
...
MPF.SunLife.simple.returns <- merge(MPF.SunLife.AE, MPF.SunLife.B, MPF.SunLife.CA, MPF.SunLife.CE, MPF.SunLife.HKT, MPF.SunLife.G, MPF.SunLife.GB, MPF.SunLife.GE, MPF.SunLife.HKB, MPF.SunLife.HKE, MPF.SunLife.MPFC, MPF.SunLife.SFP)
...
for(col in 1:length(MPF.SunLife.returns[1,])) {
  MPF.SunLife.returns[,col] <- MPF.SunLife.returns[,col] - MPF.SunLife.cs.cost[col] + MPF.SunLife.original.cost[col]
   MPF.SunLife.simple.returns[,col] <- MPF.SunLife.simple.returns[,col] - MPF.SunLife.cs.cost[col] + MPF.SunLife.original.cost[col]
}

monthly <- index(MPF.SunLife.returns)
colnames(MPF.SunLife.returns) <- c("AE", "B", "CA", "CE", "FTSE HK", "G", "GB", "GE", "HKB", "HKE", "MPFC", "SFP")
colnames(MPF.SunLife.simple.returns) <- c("AE", "B", "CA", "CE", "FTSE HK", "G", "GB", "GE", "HKB", "HKE", "MPFC", "SFP")
...
for (row in 1:length(MPF.SunLife.BIAS.month[,1])) {
  ...
  portf.rebal.fm <-
    Return.portfolio(
      na.fill(MPF.SunLife.simple.returns, 0),
...
cat("Total contribution starting from 2001/09/17: ", total.contribution, "\n")
