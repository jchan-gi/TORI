...
年率化回報 Annualized Return: ~10.07%     
算術年均回報 Mean Annual Return: ~10.11%       
累積回報（等比） Cumulative Return (Geometric): ~519.32%    
年率化標準誤差 Annualized Standard Deviation: ~9.90% (StdDev(monthly return) * sqrt(12))    
夏普比率 Sharpe Ratio: 1.0218          
索丁諾比率 Sortino Ratio: 2.1152 (MAR = 0%)      
預期損失 Expected Shortfall: 5.31% loss (0% Risk-free rate, 95% C.I.)       
...
最新結餘 Latest asset value: 1554577        
算術年均回報 Mean annual return: 9.19%      
內部回報率 Internal Rate of Return (IRR): 9.81%    
年率化標準誤差 Annualized Standard Deviation: 9.86%    
夏普比率 Sharpe Ratio: 0.9323          
索丁諾比率 Sortino Ratio: 1.9647 (MAR = 0%)    
...
MPF.BOCPT.AE <- monthlyReturn(as.xts(MPF.BOCPT$AE), type="arithmetic")
MPF.BOCPT.Bond <- monthlyReturn(as.xts(MPF.BOCPT$Bond), type="arithmetic")
MPF.BOCPT.CA <- monthlyReturn(as.xts(MPF.BOCPT$CA), type="arithmetic")
MPF.BOCPT.CE <- monthlyReturn(as.xts(MPF.BOCPT$CE), type="arithmetic")
MPF.BOCPT.ET <- monthlyReturn(as.xts(MPF.BOCPT$ET), type="arithmetic")
MPF.BOCPT.GE <- monthlyReturn(as.xts(MPF.BOCPT$GE), type="arithmetic")
MPF.BOCPT.HKE <- monthlyReturn(as.xts(MPF.BOCPT$HKE), type="arithmetic")
MPF.BOCPT.HKT <- monthlyReturn(as.xts(MPF.BOCPT$HKT), type="arithmetic")
MPF.BOCPT.JE <- monthlyReturn(as.xts(MPF.BOCPT$JE), type="arithmetic")
MPF.BOCPT.MPFC <- monthlyReturn(as.xts(MPF.BOCPT$MPFC), type="arithmetic")
MPF.BOCPT.NAT <- monthlyReturn(as.xts(MPF.BOCPT$NAT), type="arithmetic")
MPF.BOCPT.SFP <- monthlyReturn(as.xts(MPF.BOCPT$SFP), type="arithmetic")
...
MPF.BOCPT.simple.returns <- merge(MPF.BOCPT.AE, MPF.BOCPT.Bond, MPF.BOCPT.CA, MPF.BOCPT.CE, MPF.BOCPT.ET, MPF.BOCPT.GE, MPF.BOCPT.HKE, MPF.BOCPT.HKT, MPF.BOCPT.JE, MPF.BOCPT.MPFC, MPF.BOCPT.NAT, MPF.BOCPT.SFP, MPF.BOCPT.G, MPF.BOCPT.B, MPF.BOCPT.S)
...
for(col in 1:length(MPF.BOCPT.returns[1,])) {
  ...
  MPF.BOCPT.simple.returns[,col] <- MPF.BOCPT.simple.returns[,col] - MPF.BOCPT.cs.cost[col] + MPF.BOCPT.original.cost[col]
}

colnames(MPF.BOCPT.returns) <- c("AE", "Bond", "CA", "CE", "ET", "GE", "HKE", "HKT", "JE", "MPFC", "NAT", "SFP", "G", "B", "S")
colnames(MPF.BOCPT.simple.returns) <- c("AE", "Bond", "CA", "CE", "ET", "GE", "HKE", "HKT", "JE", "MPFC", "NAT", "SFP", "G", "B", "S")
