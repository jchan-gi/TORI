...
2020-03-27 v2.1.1a     
1. Fix return matrix column misalignment in Tori @ BOCPT.    
...
kable(df, "pandoc",
      col.names = c("指標 Indicator", "Tori @ BOCPT GS Pricing", "SPY", "HSI", "Golden Butterfly"))
```
...
MPF.BOCPT <-
  as.xts(
    read.zoo(
      paste(Sys.getenv("onedrive"),"/MPF/BOCPT/BOCPT.csv",sep=""),
      format = "%d/%m/%Y",
      header = TRUE,
      read = read.csv,
      na.strings = "0"
...
for (row in 1:length(MPF.BOCPT.BIAS.month[, 1])) {
  ...
  if (row == 1) {
    portf.rebal.fm <- na.fill(MPF.BOCPT.simple.returns[row, ], 0) *
      MPF.portf.weight[row, ]
  } else {
    portf.rebal.fm <-
      Return.portfolio(
        na.fill(MPF.BOCPT.simple.returns[1:row, ], 0),
...
portf.rebal.fm.dollar.gb <- Return.portfolio(
  tail(MPF.BOCPT.simple.returns,-46),
...
ggplot() +
  ...
  geom_line(aes(x = date, y = Ra, color = "pink3"), portf.dollar.monthly, 
            size =1.8) +
  labs(
    title = "Tori @ BOCPT versus SPY, QQQ, and Hang Seng Index",
    subtitle = "How Tori @ BOCPT (GS pricing) outperform common index fund",
    x = "",
    y = "Portfolio Value",
...
ggplot() +
  ...
  geom_line(aes(x = date, y = Ra, color = "pink3"),
            ...
            size = 1.8) +
  labs(
    title = "Tori @ BOCPT versus Golden Butterfly Portfolio",
    subtitle = "How Tori @ BOCPT (GS pricing) outperform golden butterfly portfolio",
    x = "",
    y = "Portfolio Value",
    color = "Portfolio"
  ) +
...
kable(df, "pandoc",
      col.names = c("指標 Indicator", "Tori @ BOCPT (Cs)", "SPY", "HSI", "Golden Butterfly"))
```
