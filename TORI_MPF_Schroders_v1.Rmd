...
年率化回報 Annualized Return: ~9.81%     
算術年均回報 Mean Annual Return: ~9.90%       
累積回報（等比） Cumulative Return (Geometric): ~492.24%    
年率化標準誤差 Annualized Standard Deviation: ~10.14% (StdDev(monthly return) * sqrt(12))    
夏普比率 Sharpe Ratio: 9.81%/10.14% = 0.9680          
索丁諾比率 Sortino Ratio: 0.5715 (MAR = 0%)      
預期損失 Expected Shortfall: 5.54 % loss (0% Risk-free rate, 95% C.I.)       
...
最新結餘 Latest asset value: 993276.9        
算術年均回報 Mean annual return: 9.46%      
內部回報率 Internal Rate of Return (IRR): 9.65%    
年率化標準誤差 Annualized Standard Deviation: 10.12%    
夏普比率 Sharpe Ratio: 0.9348          
索丁諾比率 Sortino Ratio: 0.5752 (MAR = 0%)    
...
MPF.Schroders.returns <- merge(MPF.Schroders.AE, MPF.Schroders.B, MPF.Schroders.BG, MPF.Schroders.CA, MPF.Schroders.G,
...
for (row in 1:length(MPF.Schroders.BIAS.month[,1])) {
  ...
  if (row <= 12 || MPF.Schroders.sum == MPF.Schroders.BIAS.month[row,9] ||
    ...
    if (row >= 160) {
      MPF.Schroders.BIAS.p[10] <- 0.1
      MPF.Schroders.BIAS.p[9] <- 0.6
      MPF.Schroders.BIAS.p[6] <- 0.3
    } else {
      MPF.Schroders.BIAS.p[10] <- 0.4
      MPF.Schroders.BIAS.p[9] <- 0.6
    }
    ...
    if (row >= 160) {
      ...
      MPF.Schroders.BIAS.p[6] <- MPF.Schroders.BIAS.p[6]+0.13
      MPF.Schroders.BIAS.p[9] <- MPF.Schroders.BIAS.p[9]+0.12
      MPF.Schroders.BIAS.p[10] <- MPF.Schroders.BIAS.p[10]+0.05
    } else {
      MPF.Schroders.BIAS.p[ ] <- MPF.Schroders.BIAS.month[row, ] / MPF.Schroders.sum / 10 * 7
      MPF.Schroders.BIAS.p[9] <- MPF.Schroders.BIAS.p[9]+0.12
      MPF.Schroders.BIAS.p[10] <- MPF.Schroders.BIAS.p[10]+0.18
    }
  } else {
  ...
  portf.rebal.fm <-
    Return.portfolio(
      na.fill(MPF.Schroders.returns, 0),
...
total.contribution <- 1500 * length(MPF.Schroders.returns[, 1])
