...
年率化回報 Annualized Return: ~12.14%     
算術年均回報 Mean Annual Return: ~12.12%       
累積回報（等比） Cumulative Return (Geometric): ~782.90%    
年率化標準誤差 Annualized Standard Deviation: ~11.12% (StdDev(monthly return) * sqrt(12))    
夏普比率 Sharpe Ratio: 1.0918          
索丁諾比率 Sortino Ratio: 0.6457 (MAR = 0%)      
預期損失 Expected Shortfall: 5.77% loss (0% Risk-free rate, 95% C.I.)       
...
最新結餘 Latest asset value: 1320398    
算術年均回報 Mean annual return (arithmetric): 11.80%      
內部回報率 Internal Rate of Return (IRR): 13.20%    
年率化標準誤差 Annualized Standard Deviation: 11.11%    
夏普比率 Sharpe Ratio: 1.0625          
索丁諾比率 Sortino Ratio: 0.6537 (MAR = 0%)    
...
for (col in 1:length(MPF.HSBC.returns[1,])) {
  NonNAindex <- which(!is.na(MPF.HSBC.returns[,col]))
...
#for (row in 1:13) {
  ...
  if (row == 1) {
    ...
    MPF.monthly.returns[row] <- log(sum(na.fill(this.value,0)) / 2301)
