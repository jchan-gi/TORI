...
年率化回報 Annualized Return: ~11.76%     
算術年均回報 Mean Annual Return: ~11·71%       
年率化標準誤差 Annualized Standard Deviation: ~10.57% (StdDev(monthly return) * sqrt(12))    
夏普比率 Sharpe Ratio: 11.76%/10.57% = 1.128          
索丁諾比率 Sortino Ratio: 0.6826 (MAR = 0%)      
預期損失 Expected Shortfall: 5.39% loss (0% Risk-free rate, 95% C.I.)       
...
最新結餘 Latest asset value: 1510848    
算術年均回報 Mean annual return: 11.64%      
內部回報率 Internal Rate of Return (IRR): 12.93%    
年率化標準誤差 Annualized Standard Deviation: 10.74%    
夏普比率 Sharpe Ratio: 1.0834          
索丁諾比率 Sortino Ratio: 0.6904 (MAR = 0%)    
預期損失 Expected Shortfall: 5.42% loss (0% Risk-free rate, 95% C.I.)     
...
for (row in 1:length(MPF.HSBC.BIAS.month[,1])) {
  ...
  if (MPF.HSBC.BIAS.p[1] != 0 && monthly[row] > "2019-07-01") {
    MPF.HSBC.BIAS.p[14] <- MPF.HSBC.BIAS.p[1]
    MPF.HSBC.BIAS.p[1] <- 0
  }
  
  if (MPF.HSBC.BIAS.p[5] != 0 && monthly[row] > "2019-07-01") {
    MPF.HSBC.BIAS.p[16] <- MPF.HSBC.BIAS.p[5]
    MPF.HSBC.BIAS.p[5] <- 0
  }
  
  if (MPF.HSBC.BIAS.p[11] != 0 && monthly[row] > "2019-07-01") {
    MPF.HSBC.BIAS.p[17] <- MPF.HSBC.BIAS.p[11]
