...
```{r warning=FALSE}
MPF.HSBC <- as.xts(read.zoo("F:/OneDrive/MPF/HSBC SuperTrust Plus/HSBC_MPF_SuperTrustPlus.csv", format="%d/%m/%Y", header=TRUE, na.strings="0", read = read.csv))
...
#for (row in 1:13) {
  ...
  if (row == 1) {
    ...
    MPF.monthly.asset[row,] <-
      na.fill(((this.value + 1500) / this.price * MPF.portf.weight[row, ]) / 
                this.price,0)
...
total.contribution <- 1500 * length(MPF.HSBC.simple.returns[, 1])
