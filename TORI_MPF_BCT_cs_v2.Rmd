...
kable(df, "pandoc",
      col.names = c("指標 Indicator", "Tori @ BCT GS Pricing", "SPY", "HSI", "Golden Butterfly"))
```
...
最新結餘 Latest asset value: 1686289        
算術年均回報 Mean annual return: 8.73%      
內部回報率 Internal Rate of Return (IRR): 10.22%    
年率化標準誤差 Annualized Standard Deviation: 9.13%    
夏普比率 Sharpe Ratio: 0.9557          
索丁諾比率 Sortino Ratio: 2.7683 (MAR = 0%)    
...
MPF.BCT <-
  as.xts(
    read.zoo(
      paste(Sys.getenv("onedrive"),"/MPF/BCT/BCT.csv",sep=""),
      format = "%d/%m/%Y",
      header = TRUE,
      read = read.csv,
      na.strings = "0"
...
for (col in 1:length(MPF.BCT.returns[1, ])) {
  ...
  MPF.BCT.simple.returns[, col] <-
    MPF.BCT.simple.returns[, col] - MPF.BCT.cs.cost[col] + MPF.BCT.original.cost[col]
}
...
for (col in 1:length(MPF.BCT.RSI.month[1, ])) {
  if (col != 16) {
    for (row in 1:length(MPF.BCT.RSI.month[, col])) {
      ...
      if (percentile(MPF.BCT.RSI.month[row, col]) >= (RSI_Overbuy - 1 /
                                                      length(1:row) ^ (1 / 2))) {
        MPF.BCT.RSI.p[row, col] <- 0.2
      } else {
        MPF.BCT.RSI.p[row, col] <-
          1.2 - (percentile(MPF.BCT.RSI.month[row, col]) ^ (3))
      }
    }
  } else {
    MPF.BCT.RSI.p[, col] <- 1
...
for (row in 1:length(MPF.BCT.RSI.p[, col])) {
  MPF.BCT.RSI.p[row, ] <-
    apply(MPF.BCT.RSI.p[row, ], 2, function(x)
      (x / MPF.BCT.RSI.sum[row, 1]) ^ (1 / 4))
}
MPF.BCT.RSI.sum <-
  as.xts(rowSums(MPF.BCT.RSI.p), order.by = monthly) 
...
```{r warning=FALSE}
rm(MPF.BCT.BIAS.diff,
   MPF.BCT.BIAS.diff.qu,
   MPF.BCT.BIAS.month,
   MPF.BCT.BIAS.p)
MPF.BCT.BIAS.diff <-
  (MPF.BCT.SMA.mt - MPF.BCT.SMA.hy) / MPF.BCT.SMA.hy
MPF.BCT.BIAS.diff.qu <-
  MPF.BCT.BIAS.diff - stats::lag(MPF.BCT.BIAS.diff, k = 21 * nm)
...
MPF.BCT.stock.return <-
  as.xts(rowSums(MPF.BCT.BIAS.month), order.by = monthly)
...
for (row in 1:length(MPF.BCT.BIAS.month[, 1])) {
  MPF.BCT.BIAS.month[row, ] <-
    (MPF.BCT.BIAS.month[row, ]) * MPF.BCT.RSI.p[row, ]
...
ggplot()+
  ...
  labs(title = "Tori @ BCT versus SPY, QQQ, and HSI",
       subtitle = "How Tori @ BCT (GS Price) outperform common index fund",
       x = "", y = "Portfolio Value",
...
ggplot()+
  ...
  labs(title = "Tori @ BCT versus Golden Butterfly Portfolio",
       subtitle = "How Tori @ BCT (GS Price) outperform golden butterfly portfolio",
       x = "", y = "Portfolio Value",
       color = "Portfolio") +
  theme_tq() +
  scale_y_continuous(labels = scales::dollar)
...
kable(df, "pandoc",
      col.names = c("指標 Indicator", "Tori @ BCT (cs)", "SPY", "HSI", "Golden Butterfly"))
```
