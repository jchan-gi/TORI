...
2020-03-27 v2.1.1a     
1. Fix return matrix column misalignment in Tori @ AIA.    
...
MPF.AIA <-
  as.xts(
    read.zoo(
      paste(Sys.getenv("onedrive"),"/MPF/AIA/AIA.csv",sep=""),
      format = "%d/%m/%Y",
      header = TRUE,
      read = read.csv,
      na.strings = "0"
...
for (row in 1:length(MPF.AIA.BIAS.month[, 1])) {
  ...
  if (row == 1) {
    portf.rebal.fm <- na.fill(MPF.AIA.simple.returns[row, ], 0) *
      MPF.portf.weight[row, ]
  } else {
    portf.rebal.fm <-
      Return.portfolio(
        na.fill(MPF.AIA.simple.returns[1:row, ], 0),
...
portf.rebal.fm.dollar.gb <- Return.portfolio(
  tail(MPF.AIA.simple.returns,-46),
...
kable(df, "pandoc",
      col.names = c("指標 Indicator", "Tori @ AIA", "SPY", "HSI", "Golden Butterfly"))
```
