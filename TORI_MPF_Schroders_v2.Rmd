...
2020-03-27 v2.1.1a     
1. Fix return matrix column misalignment in Tori @ Schroders.    
...
MPF.Schroders <-
  as.xts(
    read.zoo(
      paste(Sys.getenv("onedrive"),"/MPF/Schroders/Schroders.csv",sep=""),
      format = "%d/%m/%Y",
      header = TRUE,
      read = read.csv,
      na.strings = "0"
...
for (row in 1:length(MPF.Schroders.BIAS.month[, 1])) {
  ...
  if (row == 1) {
    portf.rebal.fm <- na.fill(MPF.Schroders.simple.returns[row, ], 0) *
      MPF.portf.weight[row, ]
  } else {
    portf.rebal.fm <-
      Return.portfolio(
        na.fill(MPF.Schroders.simple.returns[1:row, ], 0),
...
portf.rebal.fm.dollar.gb <- Return.portfolio(
  tail(MPF.Schroders.simple.returns,-46),
...
kable(df, "pandoc",
      col.names = c("指標 Indicator", "Tori @ Schroders", "SPY", "HSI", "Golden Butterfly"))
```
