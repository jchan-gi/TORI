...
2020-03-27 v2.1.1a     
1. Fix return matrix column misalignment in Tori @ Fidelity.    
...
for (row in 1:length(MPF.Fidelity.BIAS.month[, 1])) {
  ...
  if (row == 1) {
    portf.rebal.fm <- na.fill(MPF.Fidelity.simple.returns[row, ], 0) *
      MPF.portf.weight[row, ]
  } else {
    portf.rebal.fm <-
      Return.portfolio(
        na.fill(MPF.Fidelity.simple.returns[1:row, ], 0),
