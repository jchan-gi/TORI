...
年率化回報 Annualized Return: ~8.28%     
算術年均回報 Mean Annual Return: ~8.37%       
累積回報（等比） Cumulative Return (Geometric): ~353.62%    
年率化標準誤差 Annualized Standard Deviation: ~8.93% (StdDev(monthly return) * sqrt(12))    
夏普比率 Sharpe Ratio: 8.28%/8.93% = 0.9279          
索丁諾比率 Sortino Ratio: 0.5556 (MAR = 0%)      
預期損失 Expected Shortfall: 4.72 % loss (0% Risk-free rate, 95% C.I.)       
...
最新結餘 Latest asset value: 916627.9        
算術年均回報 Mean annual return: 8.03%      
內部回報率 Internal Rate of Return (IRR): 8.99%    
年率化標準誤差 Annualized Standard Deviation: 8.92%    
夏普比率 Sharpe Ratio: 0.9000          
索丁諾比率 Sortino Ratio: 0.5611 (MAR = 0%)    
...
for (row in 1:length(MPF.Fidelity.BIAS.month[,1])) {
  ...
  if (row > 8 && MPF.Fidelity.stock.return[row] <
      quantile(na.omit(MPF.Fidelity.stock.return), c(.40)) &&
      MPF.Fidelity.stock.return[row - 3] <
      quantile(na.omit(MPF.Fidelity.stock.return), c(.50))) {
    up <- FALSE
  }
...
#for (row in 1:13) {
  ...
  if (row == 2) {
    ...
    MPF.monthly.returns[row] <- (sum(na.fill(this.value,0)) - 1500) / 1500
...
total.contribution <- 1500 * length(MPF.Fidelity.returns[, 1])
