...
2020-03-27 v2.1.1a     
1. Fix return matrix column misalignment in Tori @ Allianz.    
...
MPF.Allianz.AE <- read.xlsx(paste(Sys.getenv("onedrive"),"/MPF/Allianz/202004/AE.xlsx", sep=""))
MPF.Allianz.AE[, 1] <- convertToDate(MPF.Allianz.AE[, 3])
MPF.Allianz.AE <-
  na.omit(MPF.Allianz.AE[(9:length(MPF.Allianz.AE[, 1])), 3:4])
rownames(MPF.Allianz.AE) <- MPF.Allianz.AE[, 1]
colnames(MPF.Allianz.AE) <- "Date"
MPF.Allianz.AE <-
  as.xts(read.zoo(MPF.Allianz.AE, format = "%Y-%m-%d"))
MPF.Allianz.AE[, 1] <- as.numeric(MPF.Allianz.AE[, 1])
storage.mode(MPF.Allianz.AE) <- "numeric"

MPF.Allianz.B <- read.xlsx(paste(Sys.getenv("onedrive"),"/MPF/Allianz/202004/B.xlsx", sep=""))
MPF.Allianz.B[, 1] <- convertToDate(MPF.Allianz.B[, 3])
MPF.Allianz.B <-
  na.omit(MPF.Allianz.B[(9:length(MPF.Allianz.B[, 1])), 3:4])
...
MPF.Allianz.CA <- read.xlsx(paste(Sys.getenv("onedrive"),"/MPF/Allianz/202004/CA.xlsx", sep=""))
MPF.Allianz.CA[, 1] <- convertToDate(MPF.Allianz.CA[, 3])
MPF.Allianz.CA <-
  na.omit(MPF.Allianz.CA[(9:length(MPF.Allianz.CA[, 1])), 3:4])
rownames(MPF.Allianz.CA) <- MPF.Allianz.CA[, 1]
colnames(MPF.Allianz.CA) <- "Date"
MPF.Allianz.CA <-
  as.xts(read.zoo(MPF.Allianz.CA, format = "%Y-%m-%d"))
MPF.Allianz.CA[, 1] <- as.numeric(MPF.Allianz.CA[, 1])
storage.mode(MPF.Allianz.CA) <- "numeric"

MPF.Allianz.FlexB <-
  read.xlsx(paste(Sys.getenv("onedrive"),"/MPF/Allianz/202004/FlexB.xlsx", sep=""))
MPF.Allianz.FlexB[, 1] <- convertToDate(MPF.Allianz.FlexB[, 3])
MPF.Allianz.FlexB <-
  na.omit(MPF.Allianz.FlexB[(9:length(MPF.Allianz.FlexB[, 1])), 3:4])
rownames(MPF.Allianz.FlexB) <- MPF.Allianz.FlexB[, 1]
colnames(MPF.Allianz.FlexB) <- "Date"
MPF.Allianz.FlexB <-
  as.xts(read.zoo(MPF.Allianz.FlexB, format = "%Y-%m-%d"))
MPF.Allianz.FlexB[, 1] <- as.numeric(MPF.Allianz.FlexB[, 1])
storage.mode(MPF.Allianz.FlexB) <- "numeric"

MPF.Allianz.G <- read.xlsx(paste(Sys.getenv("onedrive"),"/MPF/Allianz/202004/G.xlsx", sep=""))
MPF.Allianz.G[, 1] <- convertToDate(MPF.Allianz.G[, 3])
MPF.Allianz.G <-
  na.omit(MPF.Allianz.G[(9:length(MPF.Allianz.G[, 1])), 3:4])
...
MPF.Allianz.SFP <-
  read.xlsx(paste(Sys.getenv("onedrive"),"/MPF/Allianz/202004/SFP.xlsx", sep=""))
MPF.Allianz.SFP[, 1] <- convertToDate(MPF.Allianz.SFP[, 3])
MPF.Allianz.SFP <-
  na.omit(MPF.Allianz.SFP[(9:length(MPF.Allianz.SFP[, 1])), 3:4])
rownames(MPF.Allianz.SFP) <- MPF.Allianz.SFP[, 1]
colnames(MPF.Allianz.SFP) <- "Date"
MPF.Allianz.SFP <-
  as.xts(read.zoo(MPF.Allianz.SFP, format = "%Y-%m-%d"))
MPF.Allianz.SFP[, 1] <- as.numeric(MPF.Allianz.SFP[, 1])
storage.mode(MPF.Allianz.SFP) <- "numeric"

MPF.Allianz.SG <- read.xlsx(paste(Sys.getenv("onedrive"),"/MPF/Allianz/202004/SG.xlsx", sep=""))
MPF.Allianz.SG[, 1] <- convertToDate(MPF.Allianz.SG[, 3])
MPF.Allianz.SG <-
  na.omit(MPF.Allianz.SG[(9:length(MPF.Allianz.SG[, 1])), 3:4])
rownames(MPF.Allianz.SG) <- MPF.Allianz.SG[, 1]
colnames(MPF.Allianz.SG) <- "Date"
MPF.Allianz.SG <-
  as.xts(read.zoo(MPF.Allianz.SG, format = "%Y-%m-%d"))
...
MPF.Allianz <-
  head(na.locf(do.call(rbind, lapply(
    split(MPF.Allianz), tail, 1
  ))), -1)
...
MPF.Allianz.returns <-
  na.locf(do.call(cbind, lapply(MPF.Allianz, function(x)
    monthlyReturn(x, type = "log"))))

MPF.Allianz.simple.returns <-
  na.locf(do.call(cbind, lapply(MPF.Allianz, function(x)
    monthlyReturn(x, type = "arithmetic"))))

monthly <- index(MPF.Allianz.returns)

colnames(MPF.Allianz.returns) <-
  c('AE',
    'B',
    "CA",
    "FlexB",
...
colnames(MPF.Allianz.simple.returns) <-
  c('AE',
    'B',
    "CA",
    "FlexB",
...
for (col in 1:length(MPF.Allianz.RSI.month[1, ])) {
  if (col != 6) {
    for (row in 1:length(MPF.Allianz.RSI.month[, col])) {
      ...
      if (percentile(MPF.Allianz.RSI.month[row, col]) >= (RSI_Overbuy - 1 /
                                                          length(1:row) ^ (1 / 2))) {
        MPF.Allianz.RSI.p[row, col] <- 0.2
      } else {
        MPF.Allianz.RSI.p[row, col] <-
          1.2 - (percentile(MPF.Allianz.RSI.month[row, col]) ^ (3))
      }
    }
  } else {
...
MPF.Allianz.RSI.sum <-
  as.xts(rowSums(MPF.Allianz.RSI.p), order.by = monthly)

for (row in 1:length(MPF.Allianz.RSI.p[, col])) {
  MPF.Allianz.RSI.p[row, ] <-
    apply(MPF.Allianz.RSI.p[row, ], 2, function(x)
      (x / MPF.Allianz.RSI.sum[row, 1]) ^ (1 / 4))
}
MPF.Allianz.RSI.sum <-
  as.xts(rowSums(MPF.Allianz.RSI.p), order.by = monthly) 
...
```{r warning=FALSE}
rm(
  MPF.Allianz.BIAS.diff,
  MPF.Allianz.BIAS.diff.qu,
  MPF.Allianz.BIAS.month,
  MPF.Allianz.BIAS.p
)
MPF.Allianz.BIAS.diff <-
  (MPF.Allianz.SMA.mt - MPF.Allianz.SMA.hy) / MPF.Allianz.SMA.hy
MPF.Allianz.BIAS.diff.qu <-
  MPF.Allianz.BIAS.diff - stats::lag(MPF.Allianz.BIAS.diff, k = nm)
...
MPF.Allianz.stock.return <-
  as.xts(rowSums(MPF.Allianz.BIAS.month), order.by = monthly)
...
for (row in 1:length(MPF.Allianz.BIAS.month[, 1])) {
  MPF.Allianz.BIAS.month[row, ] <-
    (MPF.Allianz.BIAS.month[row, ]) * MPF.Allianz.RSI.p[row, ]
  MPF.Allianz.stock.mean <- 0
  i <- 0
  for (col in 1:length(MPF.Allianz.BIAS.month[1, ])) {
    if (col != 2 &&
        col != 3 &&
        col != 4 &&
        col != 8 && col != 10 && col != 11 && col != 12 && col != 13) {
      if (!is.na(MPF.Allianz.returns.mat[row, col]) &&
          MPF.Allianz.returns.mat[row, col] != 0) {
        MPF.Allianz.stock.mean <-
          MPF.Allianz.stock.mean + MPF.Allianz.returns.mat[row, col]
        i <- i + 1
      }
  ...
  if (row <= 12 ||
      MPF.Allianz.sum == MPF.Allianz.BIAS.month[row, 8] ||
      MPF.Allianz.sum < 1e-6 || hedge == TRUE) {
    if (row >= 194) {
      MPF.Allianz.BIAS.p[8] <- 0.5
      MPF.Allianz.BIAS.p[11] <- 0.15
      MPF.Allianz.BIAS.p[12] <- 0.35
    } else {
      MPF.Allianz.BIAS.p[8] <- 0.7
      MPF.Allianz.BIAS.p[11] <- 0.3
    }
    
  } else if (min(MPF.Allianz.stock.return[(row - 3):row]) < -0.08) {
    MPF.Allianz.BIAS.p[] <-
      MPF.Allianz.BIAS.month[row,] / MPF.Allianz.sum / 10 * 7
    if (row >= 194) {
      MPF.Allianz.BIAS.p[8] <- MPF.Allianz.BIAS.p[8] + 0.12
      MPF.Allianz.BIAS.p[11] <- MPF.Allianz.BIAS.p[11] + 0.06
      MPF.Allianz.BIAS.p[12] <- MPF.Allianz.BIAS.p[12] + 0.12
    } else {
      MPF.Allianz.BIAS.p[8] <- MPF.Allianz.BIAS.p[8] + 0.2
      MPF.Allianz.BIAS.p[11] <- MPF.Allianz.BIAS.p[11] + 0.1
    }
    
  } else {
    MPF.Allianz.BIAS.p[] <-
      MPF.Allianz.BIAS.month[row,] / MPF.Allianz.sum
  }
  ...
  if (row == 1) {
    portf.rebal.fm <- na.fill(MPF.Allianz.simple.returns[row, ], 0) *
      MPF.portf.weight[row, ]
  } else {
    portf.rebal.fm <-
      Return.portfolio(
        na.fill(MPF.Allianz.simple.returns[1:row, ], 0),
...
portf.rebal.fm.dollar.gb <- Return.portfolio(
  tail(MPF.Allianz.simple.returns,-42),
  weight = tail(MPF.portf.weight,-42),
  geometric = TRUE,
  rebalance_on = "months"
)
...
ggplot() +
  scale_color_manual(
    labels = c("SPY", "QQQ", "HSI", "TORI @ Allianz"),
  ...
  geom_line(aes(x = date, y = Ra, color = "pink3"), portf.dollar.monthly, size =
              1.8) +
  labs(
    title = "TORI @ Allianz versus SPY, QQQ, and Hang Seng Index",
    subtitle = "How TORI @ Allianz outperform common index fund",
    x = "",
    y = "Portfolio Value",
...
ggplot() +
  scale_color_manual(
    labels = c("Golden Butterfly", "TORI @ Allianz"),
    values = c(4, "pink3")
  ) +
  ...
  geom_line(aes(x = date, y = Ra, color = "pink3"),
            ...
            size = 1.8) +
  labs(
    title = "TORI @ Allianz versus Golden Butterfly Portfolio",
    subtitle = "How TORI @ Allianz outperform golden butterfly portfolio",
    x = "",
    y = "Portfolio Value",
    color = "Portfolio"
  ) +
...
cat("Total contribution starting from 2001/04/01: ",
    total.contribution,
    "\n")
...
kable(df, "pandoc",
      col.names = c("指標 Indicator", "Tori @ Allianz", "SPY", "HSI", "Golden Butterfly"))
```
